 ...
 * Date: 24.01.2006
 * Time: 14:16:38
 * $Header: /cvsroot/sikher/sikher_java/src/sikher/visual/controls/SearchSikherControl.java,v 1.1 2006/04/21 13:04:12 mamonts Exp $
 * $Log: SearchSikherControl.java,v $
 * Revision 1.1  2006/04/21 13:04:12  mamonts
 * First commtin
 *
 * Description:
 */
public class SearchSikherControl extends SikherComponent {
    private XPathSearcher _FSearcher;

    public SearchSikherControl( XPathSearcher searcher, SikherIni ini, StatusBar statusbar ){
    ...
    public String getControlName() {
        return SikherProperties.getString("control.search.name");
    }
    ...
    private void _FinitComponents(){
        ...
        JLabel _label = new JLabel( SikherProperties.getString("control.search.label.search") );
        _const.gridx = 0;
        _const.gridy = 0;
        ...
        jtf_Text.setFont( new Font( FSikherIni.getDefaultFontName(), Font.PLAIN, 14 ) );
        _const.gridx = 1;
        _const.fill = GridBagConstraints.HORIZONTAL;
        _const.gridwidth = 2;
        _p.add( jtf_Text, _const );

        KeyboardPanel _keyboard = new KeyboardPanel( jtf_Text, FSikherIni );
        _const.gridx = 0;
        _const.gridy = 1;
        ...
        _p.setBorder(BorderFactory.createTitledBorder( SikherProperties.getString("control.search.panel.options")));
        ...
        _label = new JLabel( SikherProperties.getString("control.search.label.page") );
        ...
        jrb_All = new JRadioButton( SikherProperties.getString("control.search.radio.pageall") );
        jrb_All.setSelected(true);
        _pagePanel.add( _FcreatePagePanel( jrb_All, _bg, new JComponent[]{} ) );

        jrb_Range = new JRadioButton( SikherProperties.getString("control.search.radio.pagerange") );
        jspr_From = new JSpinner();
        jspr_To = new JSpinner();
        _pagePanel.add( _FcreatePagePanel( jrb_Range, _bg, new JComponent[]{
                new JLabel( SikherProperties.getString("control.search.label.from") ),
                jspr_From,
                new JLabel( SikherProperties.getString("control.search.label.to") ),
                jspr_To
        } ) );

        jrb_Exact = new JRadioButton( SikherProperties.getString("control.search.radio.pageexact") );
        jspr_Exact = new JSpinner();
        _pagePanel.add( _FcreatePagePanel( jrb_Exact, _bg, new JComponent[]{jspr_Exact} ) );

        jrb_Random = new JRadioButton( SikherProperties.getString("control.search.radio.pagerandom") );
    ...
    private class FindAction extends AbstractAction{
        public FindAction(){
            putValue( NAME, SikherProperties.getString("control.search.button.find") );
        }
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            new Thread( new Runnable(){
                public void run() {
                    ...
                    if( _book == null || _author == null || _raag == null ){
                        ...
                        SwingUtilities.invokeLater( new Runnable(){
                            public void run() {
                                showErrorMessage(
                                        SikherProperties.getString("control.search.operation.search"),
                                        SikherProperties.getString("control.search.error.noinfo"));
                            }
                        } );
                        return;
                    }
                ...
                public void done( int found ){
                    final String _resMessage = SikherProperties.getString("control.search.status.resultsfound",
